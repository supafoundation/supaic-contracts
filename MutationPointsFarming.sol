...
// File: mutationpointsfarming.sol
...
contract MutationPointsFarming is ReentrancyGuard,AccessControl {
 ...
 MutationPointsGetter SUPAVirus= MutationPointsGetter(SUPAVirusAddr); //SUPACell clone contract address
  MutationPointsGetter SUPACell= MutationPointsGetter(SUPACellAddr); //SUPAVirus clone contract address
    mapping(address => bool) public whitelistedContract;
    mapping(uint=>uint) public lastFarmedVirus;
    mapping(uint=>uint) public lastFarmedCell;
uint multiplier=20;
...
bool public isPaused=false;
 ...
 function updateWL(int RDnew, int GRnew, int BKnew, int BUnew, int WHnew, int ORnew, int PRnew, address playerAddress) public  nonReentrant{
           require(
           isPaused==false ,
            "Updating is Paused."
        );
        ...
        require(
           isPaused==false ,
            "Farming is Paused."
                ...
                if(organism==0){
                uint totalSUPAVirus=IERC721(SUPAVirusAddr).balanceOf(msg.sender);
                    if (end>totalSUPAVirus){
                        end=totalSUPAVirus;
                    }
                for(uint256 i = start; i < end; i++) {
               uint tokenID= IERC721Enumerable(SUPAVirusAddr).tokenOfOwnerByIndex(msg.sender,i);
                    farm(tokenID,0);
                 }
                }
                else{
                   uint totalSUPACell=IERC721(SUPACellAddr).balanceOf(msg.sender);
                    if (end>totalSUPACell){
                        end=totalSUPACell;
                    }
                 for(uint256 i = start; i < end; i++) {
                   uint tokenID= IERC721Enumerable(SUPACellAddr).tokenOfOwnerByIndex(msg.sender,i);
                    farm(tokenID,1);
                 }
                }
    ...
    function farm(uint tokenId, uint organism ) public  nonReentrant{
        ...
        if(organism==0){
                //SUPAVirus
           require( IERC721(SUPAVirusAddr).ownerOf(tokenId)==msg.sender);
    ...
    (uint RDnew, uint GRnew, uint BKnew, uint BUnew, uint WHnew, uint ORnew, uint PRnew) =  SUPAVirus.NFTMutationPoints(tokenId);
       ...
       require( IERC721(SUPACellAddr).ownerOf(tokenId)==msg.sender);
