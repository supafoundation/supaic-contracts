...
contract SUPAOrganism is ERC721, ERC721URIStorage, ERC721Enumerable, Pausable, AccessControl, ERC2981ContractWideRoyalties,ReentrancyGuard {
    ...
    address public OGNFTAddr=0x7BB15b49059270517a72f949A8b3f146740111a5;
    address public SUPAVirusAddr=0xE186A363691fDA04423D84689aE44995070C9B6e;
    address public SUPACellAddr=0x1c3E92b4E9DCCb4Cd1C94DD0ba6eb93835Fc8A45;

    uint256 private mintSize = 10000;
    IERC721 private OGNFT = IERC721(OGNFTAddr); //can use standard openzeppelin erc721 contract to test
    mintOrganism SUPAVirus= mintOrganism(SUPAVirusAddr); //clone of SUPACell.sol
    mintOrganism SUPACell= mintOrganism(SUPACellAddr); //SUPACell.sol
    IERC20 private supaToken = IERC20(0x86A677e61cd7e2E7020F8D0c9f75AED099642d5a); //can use standard openzeppelin erc20 contract to test
    string private base = "https://supafoundation.b-cdn.net/SUPAOrganismjson/";
    mapping(string => Counters.Counter) public seenNoncesDiscord; 
      mapping(uint => Counters.Counter) public seenNoncesOG;
       mapping(uint => bool) public seenNoncesTokenId;
        ...
        uint private single=2000000000000000000000;
        uint private double=3500000000000000000000;
          uint private combo=6000000000000000000000;
    ...
    constructor() ERC721("SUPA Organism", "SUPAORGANISM") {
       ...
       preRelease=1642687200;
        publicRelease=1642687200+86400;
        identityRelease=1642687200+86400*3;
    ...
    function setNFTAddress(address newOG, address newVirus, address newCell)  public onlyRole(DEFAULT_ADMIN_ROLE){
        OGNFTAddr=newOG;
        SUPAVirusAddr=newVirus;
        SUPACellAddr=newCell;
    }
     function setMintPrice(uint singleNew, uint doubleNew, uint comboNew)  public onlyRole(DEFAULT_ADMIN_ROLE){
       single=singleNew;
        double=doubleNew;
        combo=comboNew;
    }
    ...
    function verifyDiscordUser(string memory discordUser, bytes memory sig) internal virtual returns (bool) {
    ...
    function mintWithGenesis(string memory discordUser,  uint qty, bytes memory sig) public payable  nonReentrant {
        require(_tokenIdCounter.current() + qty <= mintSize, "Mint limit reached");
                require(qty>0 && qty<5 && qty!=3, "Invalid mint amount");
         require(verifyDiscordUser(discordUser, sig),"Invalid signature provided");
        ...
        require(block.timestamp>preRelease,"Not released yet");
           ...
           if(qty==4){
               require(
           supaToken.allowance(msg.sender,address(this)) >= combo,
            "Please make sure you have approved SUPA Token to purchase NFT"
          );
          supaToken.transferFrom(msg.sender, address(this), combo);
        ...
        _setTokenURI(_tokenIdCounter.current(), string(abi.encodePacked(Strings.toString(_tokenIdCounter.current()), '.json')));
         emit mintSuccessful(_tokenIdCounter.current());
           ...
           if(qty==2){
                require(
           supaToken.allowance(msg.sender,address(this)) >= double,
            "Please make sure you have approved SUPA Token to purchase NFT"
          );
          supaToken.transferFrom(msg.sender, address(this), double);   
           } else { 
                 require(
           supaToken.allowance(msg.sender,address(this)) >= single,
            "Please make sure you have approved SUPA Token to purchase NFT"
          );
          supaToken.transferFrom(msg.sender, address(this), single); 
           }  
          for (uint256 i = 0; i < qty; i++) {
         _tokenIdCounter.increment();
        _safeMint(msg.sender, _tokenIdCounter.current());
     ...
     function mintWithOGNFT(uint OGTokenID, uint qty) public payable  nonReentrant {
        require(_tokenIdCounter.current() + qty <= mintSize, "Mint limit reached");
         require(qty>0 && qty<5 && qty!=3, "Invalid mint amount");
        require(seenNoncesOG[OGTokenID].current()<2,"Limit of 2 mints per OG Vortex NFT");
     ...
     function mint(uint qty) public payable  nonReentrant {
          require(qty>0 && qty<5 && qty!=3, "Invalid mint amount");
